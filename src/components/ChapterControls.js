...
const ChapterControls = ({ onContentChange }) => {
  const { user } = useAuth();
  ...
  const handlePersonalize = async () => {
    ...
    try {
      // Get current page content with more specific selectors for Docusaurus docs
      let content = '';

      // Try to find the main document content area - more specific to Docusaurus structure
      const contentSelectors = [
        '.markdown > div',           // Main markdown content container
        '.markdown article',         // Article inside markdown
        '.markdown',                 // Direct markdown content
        '.theme-doc-markdown',       // Docusaurus specific markdown class
        '.theme-doc-content',        // Docusaurus doc content wrapper
        '.doc-content',              // Docusaurus content area
        'article',                   // Generic article tag
        '[data-testid="doc-markdown"]', // Docusaurus specific test ID
        '[class*="markdown"]',       // Classes containing markdown
        '.container'                 // Container that might have the content
      ];

      for (const selector of contentSelectors) {
        const contentElement = document.querySelector(selector);
        if (contentElement) {
          // Get text content but exclude common UI elements
          const clone = contentElement.cloneNode(true);

          // Remove common UI elements that are not part of the main content
          const elementsToRemove = [
            '.theme-edit-this-page',    // Edit link
            '.theme-last-updated',      // Last updated info
            '.theme-doc-breadcrumbs',   // Breadcrumbs
            '.pagination-nav',          // Pagination
            '.theme-admonition',        // Admonitions might be part of content, keep them
            ...
            '[class*="footer"]',        // Footer classes
            '[class*="menu"]'           // Menu classes
          ];

          elementsToRemove.forEach(removeSelector => {
            const elements = clone.querySelectorAll(removeSelector);
            elements.forEach(el => el.remove());
          });

          const textContent = clone.innerText || clone.textContent || '';

          // Only use this content if it's substantial (more than 200 characters to ensure real content)
      ...
      if (content.length < 200) {
        // Look for content within the main article/document area
        ...
        if (mainContent) {
          ...
          const uiElements = clone.querySelectorAll('nav, .navbar, .sidebar, .menu, .header, .footer, [class*="sidebar"], [class*="nav"], [class*="header"], [class*="footer"], [class*="menu"]');
  ...
  const handleReset = () => {
    // Reset to original content by re-rendering
